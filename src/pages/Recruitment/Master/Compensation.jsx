...
const Compensation = ({ notify, enterLoading, exitLoading, loadings }) => {
  ...
  const handleEdit = () => {
    // enterLoading(1)
    return apiProvider.editCompensationMode(user?._id, user)
      .then(res => {
        if (res.isSuccess) {
          clearData()
          getData()
  ...
  const handleDelete = (id, status) => {
    return apiProvider.editCompensationMode(id, { status: status })
      .then(res => {
        if (res.isSuccess) {
          clearData()
          getData()
  ...
  const handleSubmit = () => {
    ...
    // enterLoading(1)
    return apiProvider.createCompensationMode(user)
      .then(res => {
        // exitLoading(1)
        if (res.isSuccess) {
          clearData()
  ...
  return (
    <div>
      <Card>
        <div className='font-bold'> Add Compensation </div>
        <div className='grid grid-cols-1 lg:grid-cols-3 sm:grid-cols-2 mt-4'>
          <div className="col-span-1">
            <Input
              label={'Compensation Name'}
              placeHolder={'Enter Compensation Name'}
              name="name"
              value={user?.name}
        ...
        <div className="flex justify-end mt-3">
          <Button
            title={edit ?"Update Compensation":'Add Compensation'}
