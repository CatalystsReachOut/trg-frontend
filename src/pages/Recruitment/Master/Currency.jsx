...
const Currency = ({ notify, enterLoading, exitLoading, loadings }) => {
  ...
  const handleEdit = () => {
    // enterLoading(1)
    return apiProvider.editCurrency(user?._id, user)
      .then(res => {
        if (res.isSuccess) {
          clearData()
          getData()
  ...
  const handleDelete = (id, status) => {
    return apiProvider.editCurrency(id, { status: status })
      .then(res => {
        if (res.isSuccess) {
          clearData()
          getData()
  ...
  const handleSubmit = () => {
    ...
    // enterLoading(1)
    return apiProvider.createCurrency(user)
      .then(res => {
        // exitLoading(1)
        if (res.isSuccess) {
          clearData()
  ...
  return (
    <div>
      <Card>
        <div className='font-bold'> Add Currency </div>
        ...
        <div className="flex justify-end mt-3">
          <Button
            title={edit ?"Update Currency":'Add Currency'}
