...
const Login = ({ notify, enterLoading, exitLoading, loadings }) => {
    ...
    apiProvider
      ...
      .then((res) => {
        if (res.isSuccess) {
          ...
          // sessionStorage.setItem('band',res.band)
  ...
  return (
    <>
      <div className="font-medium text-2xl leading-[2.5rem]">
        Personal Sign in
      </div>
      <Inputfield
        prefix={<MdEmail />}
      ...
      <Link
        to="/forgotpassword"
        className="capitalize font-medium text-ternary leading-[19px] self-end mt-[0.5rem] hover:underline"
      >
        Forgot password ?
      </Link>
      ...
      <div className="text-Small/Title/Small text-black/50 text-lg self-center">
        Looking for a job?
        <Link to="/signup" className="text-ternary text-lg hover:underline">
          {" "}
          Signup{" "}
        </Link>
      </div>
